...
class Enemy(pg.sprite.Sprite):
    ...
    def handle_animation(self):
        ...
        if self.attack_mode and self.charge_power > 0:
            ...
            self.magic_balls.add(Magicball(magicball_position, self.side, self.charge_power))
            self.charge_power = 0
...
class Magicball(pg.sprite.Sprite):
    def __init__(self, coord, side, power):
        ...
        self.image = load_image('images/earth monk/magicball.png', 200, 150)
        if self.side == 'left':
...
class Fireball(pg.sprite.Sprite):
    def __init__(self, coord, side, power):
        ...
        self.image = load_image(f'images/{self.folder}/fireball.png', 200, 150)
        if self.side == 'right':
            self.image = pg.transform.flip(self.image, True, False)
...
class Menu:
    def __init__(self):
        ...
        self.menu.add.button('For one player', self.start_one_player_game)
        self.menu.add.selector('Enemy: ', [('lightning wizard', 1), ('earth monk', 2), ('Random', 3)],
                               onchange=self.set_enemy)
        self.menu.add.button('For two players', self.start_two_player_game)
        self.menu.add.selector('First player: ', [('lightning wizard', 1), ('earth monk', 2)],
                               onchange=self.set_left_player)
        self.menu.add.selector('Second player: ', [('lightning wizard', 1), ('earth monk', 2)],
    ...
    def set_enemy(self, value, selected):
        if value in (1, 2):
...
class Game:
    def __init__(self, mode,wizards):
        ...
        #
        #         self.GET_GESTURE = pg.USER_EVENT + 1
    ...
    def draw(self):
        ...
        pg.draw.rect(self.screen, 'green', (30, 15, self.player.hp, 15), 0, 10)
        pg.draw.rect(self.screen, 'green', (SCREEN_WIDTH - self.enemy.hp - 30, 15, self.enemy.hp, 15), 0, 10)

        if self.win == self.player:
            text = text_render("ПОБЕДА")
            text_rect = text.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2))
            self.screen.blit(text, text_rect)
            text2 = text_render("left wizard")
            ...
            text_rect = text.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2))
            self.screen.blit(text, text_rect)
            text2 = text_render("right wizard")
            text_rect2 = text2.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 - 100))
            self.screen.blit(text2, text_rect2)
        pg.display.flip()
